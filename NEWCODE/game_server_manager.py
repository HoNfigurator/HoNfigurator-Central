...
from TCP.packet_handler import handle_clients
from TCP.masterserver_connector import MasterServerHandler
from TCP.chatserver_connector import ChatServerHandler
from cogs.exceptions import ServerConnectionError, AuthenticationError
import phpserialize
from cogs.custom_print import my_print, logger, get_script_dir
...
class GameServerManager:
    def __init__(self, global_config):
        ...
        self.server_start_semaphore = asyncio.Semaphore(self.global_config['hon_data']['max_num_svrs_starting_at_once'])  # 2 max servers starting at once
        self.game_servers = {}
        self.client_connections = {}
        # Initialize a Commands object for sending commands to game servers
        self.commands = Commands(self.game_servers, self.client_connections, self.send_svr_command)
        # Create an event and task for handling input commands
        stop_event = asyncio.Event()
        input_task = asyncio.create_task(self.commands.handle_input(stop_event))
        # Create game server instances
        for id in range (1,self.global_config['hon_data']['server_total']+1):
            port = global_config['hon_data']['starting_game_port'] + id
    ...
    async def send_svr_command(self, command, game_server_port, command_data):
        ...
        if command == "shutdown":
            ...
            if game_server:
                res = await game_server.schedule_shutdown_server(client_connection,command_data)
                self.remove_game_server(game_server)
        else:
            ...
            if client_connection:
                ...
                my_print(f"No client connection found for port {game_server_port}")
    ...
    async def start_game_server_listener(self,host,game_server_to_mgr_port):
        ...
        logger.info(f"[*] HoNfigurator - Listening on {host}:{game_server_to_mgr_port}")
    ...
    def create_game_server(self, game_server_port):
        ...
        Returns:
            None
        """
        id = game_server_port - self.global_config['hon_data']['starting_game_port']
        game_server = GameServer(id, game_server_port, self.global_config)
    ...
    def find_next_available_ports(self):
        ...
        """
        starting_game_port = self.global_config['hon_data']['starting_game_port']
        total_allowed_servers = self.global_config['hon_data']['server_total']
    ...
    def create_dynamic_game_server(self):
        ...
        if game_port is not None:
            ...
            my_print(f"Game server created at game_port: {game_port}")
        else:
            my_print("No available ports for creating a new game server.")
    ...
    async def start_game_servers(self):
        ...
        async def start_game_server_with_semaphore(game_server):
            async with self.server_start_semaphore:
                ...
                if started:
                    my_print(f"Game server with port {game_server.port} started successfully.")
                    
                    # Wait for the game server to finish loading
                    while game_server.game_state._state['status'] is None:
                        await asyncio.sleep(1)
                    
                    my_print(f"Game server with port {game_server.port} finished loading.")
                else:
                    my_print(f"Game server with port {game_server.port} failed to start.")
